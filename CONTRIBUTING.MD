...
| **Publish**      | **Only** on `main` after a successful build    | Downloads artifact. Creates `.npmrc` for the artifact. Authenticates with **NpmAuthenticate**. Skips if the version already exists; otherwise publishes. |
